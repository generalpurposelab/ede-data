...
class Check:
    ...
    """
    def __init__(self, input_dir, api_key, model, target_language, size):
    ...
    def run(self):
        # 1) Check whether input_schema.csv is present 
        schema_file = 'input/input_schema.csv'
        if not os.path.exists(schema_file):
            print("Error: input_schema.csv is not present.")
            return
    ...
    def confirm(self):
        print(f"instruct-global is initialized to create a {self.size} row dataset in {self.target_language} (language code: {self.language_code})")
        proceed = input("Do you want to proceed? (yes/no): ")
        if proceed.lower() not in ["yes", "y"]:
            print("instruct-global cancelled")
            return False
        return True